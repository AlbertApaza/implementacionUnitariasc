...
namespace ProyectoConstruccion_APAZA_CUTIPA.Repositories
{
    ...
    {
        private readonly string _conexion = ConfigurationManager.ConnectionStrings["MySqlConexion"].ConnectionString;

        public clsUsuario Autenticar(string correo, string contrasena, out string mensajeError, out bool esAprobado)
        {
        ...
        public bool VerificarCorreoExistente(string correo, out bool estaAprobado, out string mensaje)
        {
            estaAprobado = false;
        ...
        public bool Registrar(clsUsuario usuario, out string mensajeError)
        {
            mensajeError = string.Empty;
            using (MySqlConnection con = new MySqlConnection(_conexion))
            {
        ...
        public clsUsuario ObtenerOInsertarUsuarioGoogle(string email, string nombre, out bool necesitaAprobacion, out string mensajeProceso) // Nombres de parámetro ajustados
        {
        ...
        public clsUsuario ProcesarUsuarioGoogleApi(string email, string name, out bool esNuevoUsuarioPendiente, out bool yaAprobado, out string mensaje)
        {
        ...
        public List<clsUsuario> ObtenerSolicitudesPendientes()
        {
        ...
        // Implementación de AprobarSolicitud - Mueve la lógica de DB
        public bool AprobarSolicitud(int id, string rolAsignado)
        {
            using (MySqlConnection con = new MySqlConnection(_conexion))
            {
                con.Open();
        ...
        public bool GuardarTokenFcmParaUsuario(int usuarioId, string tokenFcm)
        {
            using (MySqlConnection con = new MySqlConnection(_conexion))
            {
                con.Open();
